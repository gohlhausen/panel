...
#define N 16384
#define maxbins 256
#define detailbins 16
#define buffers 32 
#define buffer_frames  512
#define HzperBin 2.928987789
#define MINSCALAR 0.10
#define origMaxFactor 0.5
#define MaxFactor 0.0
...
double maxmL;
double maxmR;
double lastmaxm2=0.0;
int maxmLbin;
int maxmRbin;
double PanelBinsL[64][maxbins+4];
double PanelBinsR[64][maxbins+4];
double SummaryBinsL[64][maxbins+4];
double SummaryBinsR[64][maxbins+4];
struct Color MaxColor[maxbins+4];
int MaxAge[maxbins+4];
int MaxY[maxbins+4];
int CurrentPanelBin;
int CurrentSummaryBin;
...
int bin,i;
	...
	for(i=0;i<64;i++){
	   for(bin=0;bin<maxbins;bin++){
		...
		SummaryBinsL[i][bin]=(double)bin/(double)maxbins;
		SummaryBinsR[i][bin]=1.0-((double)bin/(double)maxbins);
		}
       	    PanelBinsL[i][maxbins+2]=0;
	    PanelBinsR[i][maxbins+2]=0;
       	    SummaryBinsL[i][maxbins+2]=0;
	    SummaryBinsR[i][maxbins+2]=0;
	}
	   for(bin=0;bin<maxbins;bin++){
		MaxY[bin]=0;
		MaxAge[bin]=0;
...
struct Color PixelColorOrig(double volL, double volR, double maxL, double maxR,int Brighter){
/*
Right=240 hue
Left=0 Hue
Sat=100 always
...
void PrintFreq()
{
  ...
  void Run() {
	...
	// Summary
      for (y = 0; y < height-starty-16; ++y) {
	currow=(64+(CurrentSummaryBin-y-4))%64;
        for (x = 0; x < width; ++x) {
	  pcolor=PixelColor(SummaryBinsL[currow][x+1],SummaryBinsR[currow][x+1],SummaryBinsL[currow][maxbins+2],SummaryBinsR[currow][maxbins+2],((x+8)%25==0)?(BRIGHTNESS):(0));
          UPOffscreenCanvas->SetPixel(x, y+starty+16, pcolor.r,pcolor.b,pcolor.g); 
        }
      }
	...
	// disable highlight in top freq bar
	bool vmax=false;
		MaxAge[x+1]=MaxAge[x+1]+1;
		if(MaxY[x+1]<=cheight){
		//MaxColor[x+1]=pcolor;
		MaxAge[x+1]=0;
		MaxY[x+1]=cheight+1;
		}
		if(MaxAge[x+1]>50){MaxY[x+1]=MaxY[x+1]-1;}
	for(y=0;y<starty;y++){
	  if ((starty-y)<=cheight){
            //UPOffscreenCanvas->SetPixel(x, y, (vmax?192:0),(vmax?192:0),(vmax?192:128)); 
            //UPOffscreenCanvas->SetPixel(x, y, ((x+8)%25==0)?(BRIGHTNESS):(vmax?192:0),((x+8)%25==0)?(BRIGHTNESS):(vmax?192:0),((x+8)%25==0)?(BRIGHTNESS):(vmax?192:128)); 
            UPOffscreenCanvas->SetPixel(x, y, ((x+8)%25==0)?(vmax?192-BRIGHTNESS:0):(vmax?192:0),((x+8)%25==0)?(vmax?192-BRIGHTNESS:0):(vmax?192:0),((x+8)%25==0)?(vmax?192-BRIGHTNESS:128):(vmax?192:128-BRIGHTNESS)); 
	  } else {
            UPOffscreenCanvas->SetPixel(x, y,((x+8)%25==0)?(BRIGHTNESS):(0),((x+8)%25==0)?(BRIGHTNESS):(0),((x+8)%25==0)?(BRIGHTNESS):(0)); 
	  }
	// Orange fallign dots
	if(y==(starty-MaxY[x+1])){
            UPOffscreenCanvas->SetPixel(x, y,128,0,96); 
            //UPOffscreenCanvas->SetPixel(x, y,MaxColor[x+1].r,MaxColor[x+1].g,MaxColor[x+1].b); 
	}
...
double maxv=-99999999;
	...
	retval=retval/(abs(endb-startb)/10+1);
	return(retval);
	//return(log10(1.0+(maxv*10)));
	//return(maxv);
}
void makesummarybins(int summarycount){
int bins;
int cbin;
double sumL;
double sumR;
...
startpanelbin=(CurrentPanelBin+(63-summarycount))%64;
	for(bins=0;bins<maxbins+1;bins++){
		sumL=0;
		sumR=0;
		for(cbin=startpanelbin;cbin<startpanelbin+summarycount;cbin++){
			if(sumL<PanelBinsL[cbin%64][bins]){
				sumL=PanelBinsL[cbin%64][bins];}
			if(sumR<PanelBinsR[cbin%64][bins]){
				sumR=PanelBinsR[cbin%64][bins];}
		}
		SummaryBinsL[CurrentSummaryBin][bins]=sumL;
		SummaryBinsR[CurrentSummaryBin][bins]=sumR;
...
//lmdiff=max(maxL,MINSCALAR)-lastmaxm2;
//bins[maxbins+2]=lastmaxm2+(lmdiff*0.005);
bins[maxbins+2]=max(maxL,MINSCALAR);
lastmaxm2=bins[maxbins+2];
}
...
int doFFT(int startbuffer, char *buffer[])
{
	...
	maxmRbin=0;
	for (int i = N/2; i>0;i--) {
		mL[i]=adjustment[i]*sqrt(yL[i][IMAG]*yL[i][IMAG]+yL[i][REAL]*yL[i][REAL])/(double)(N/2);
		mR[i]=adjustment[i]*sqrt(yR[i][IMAG]*yR[i][IMAG]+yR[i][REAL]*yR[i][REAL])/(double)(N/2);
		if(mL[i]>maxmL){
			maxmL=mL[i];
			maxmLbin=i;
			}
...
//	CurrentPanelBin=(CurrentPanelBin+((startbuffer%3)==0?1:0))%64;
	...
	if((CurrentPanelBin%4)==0){
		CurrentSummaryBin=(CurrentSummaryBin+1)%64;
		makesummarybins(4);
		makebins(SummaryBinsL[CurrentSummaryBin],mL);
		makebins(SummaryBinsR[CurrentSummaryBin],mR);
	}
    return 0;
}
