...
public:
  ...
  virtual void SetRowAddress(GPIO *io, int row) {
    ...
    for (int r = 7; r >= 0; r--) {
      ...
      io->SetBits(dck_);
      io->SetBits(dck_);
      //usleep(1);  // A 1 us delay was required on the board to allow clocking to work properly
      io->ClearBits(dck_);
      }
...
//FM6126 panels, which are apparently equivalent to the ICN2038S/2045
...
/*static*/ void Framebuffer::InitializePanels(GPIO *io,
  ...
  if (strncasecmp(panel_type, "fm6126", 6) == 0) {
    ...
    fprintf(stderr, "Known panel type '%s'; typo ?\n", panel_type);
